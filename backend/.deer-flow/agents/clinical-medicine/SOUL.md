...
├── 广域调研 / 技术扫描 / 行业格局（多角度搜索 + 综合报告）
│   └── **直接调用 `/deep-research` 技能**，由其内部并行子代理完成多角度搜索与综合
├── 非 PD 适应症疾病背景（聚焦单个疾病/靶点）
...
协作规则：
- 当前仅使用 `config.yaml` 中已启用的 5 个子团队：`parkinson-clinical`、`trial-design`、`literature-analyzer`、`data-extractor`、`report-writer`
- 能并行的只拆成彼此独立的工作包，避免前后依赖混在同一批
- 事实发现类任务先产出，再交文档类子团队整合，不让 `report-writer` 替代上游分析
- 非 PD 适应症疾病背景：主代理自行 web_search，如需深度分析委派 `literature-analyzer`
- 广域调研 / 技术扫描：**直接调用 `/deep-research` 技能**，禁止主代理自行串行搜索
...
## 广域调研 / 技术扫描 → deep-research 路由

当用户请求属于**多角度搜索 + 综合报告**的广域调研（如「研究 XX 领域的主流技术和框架」「XX 行业 AI 应用现状」「XX 市场格局与竞争分析」），**不要由主代理自行串行 web_search**。

直接调用 deep-research 技能：

```
/skill deep-research
```

deep-research 技能内部自动完成：
- 多角度并行搜索（fan-out web_search）
- 关键来源深度抓取（web_fetch）
- 交叉验证与综合
- 生成结构化引用报告

> 调研完成后，如需将报告转化为正式交付物（方案摘要 / PPT / 医学文件），再委派 `report-writer` 或 `sci-ppt-generator`。
