...
class AioSandbox(Sandbox):
    ...
    def read_file(self, path: str) -> str:
        ...
        try:
            ...
            # Diagnostic: log which files are successfully read from TOS mounts
            if any(p in path for p in ("/mnt/user-data/uploads", "/mnt/user-data/outputs", "/mnt/user-skills")):
                logger.info(
                    "TOS_MOUNT_DIAG read_file path=%r → SUCCESS (%d chars)",
                    path,
                    len(content),
                )
            return content
            ...
            if any(p in path for p in ("/mnt/user-data/uploads", "/mnt/user-data/outputs", "/mnt/user-skills")):
                logger.info(
                    "TOS_MOUNT_DIAG read_file path=%r → FAILED: %s",
                    path,
                    e,
                )
    ...
    def download_file(self, path: str, *, max_bytes: int | None = None) -> bytes:
        ...
        Args:
            ...
            max_bytes: Optional caller-imposed size cap. Combined with the
                builtin _MAX_DOWNLOAD_SIZE; the lower of the two applies.

        Raises:
            PermissionError: path traversal or outside VIRTUAL_PATH_PREFIX.
            OSError(errno=EFBIG): file exceeds effective cap.
            OSError: other read failures.
        """
        ...
        builtin_cap = _MAX_DOWNLOAD_SIZE
            ...
            try:
                ...
                for chunk in self._client.file.download_file(path=path):
                    ...
                    if total > effective_cap:
                        raise OSError(
                            errno.EFBIG,
                            f"File exceeds maximum download size of {effective_cap} bytes",
                            path,
                        )
                    chunks.append(chunk)
    ...
    def list_dir(self, path: str, max_depth: int = 2) -> list[str]:
        ...
        with self._lock:
            try:
                cmd = f"find {shlex.quote(path)} -maxdepth {max_depth} -type f -o -type d 2>/dev/null | head -500"
                result = self._client.shell.exec_command(command=cmd, no_change_timeout=self._DEFAULT_NO_CHANGE_TIMEOUT)
                output = result.data.output if result.data else ""
                if output:
                    entries = [line.strip() for line in output.strip().split("\n") if line.strip()]
                    # Diagnostic: log what the VeFaas TOS mount actually exposes
                    if any(p in path for p in ("/mnt/user-data/uploads", "/mnt/user-data/outputs", "/mnt/user-skills", "/mnt/user-data/workspace")):
                        logger.info(
                            "TOS_MOUNT_DIAG list_dir path=%r max_depth=%d → %d entries: %s",
                            path,
                            max_depth,
                            len(entries),
                            entries[:50] if len(entries) > 50 else entries,
                        )
                    return entries
                # Diagnostic: log empty listings too
                if any(p in path for p in ("/mnt/user-data/uploads", "/mnt/user-data/outputs", "/mnt/user-skills", "/mnt/user-data/workspace")):
                    logger.info(
                        "TOS_MOUNT_DIAG list_dir path=%r max_depth=%d → EMPTY (output=%r)",
                        path,
                        max_depth,
                        output,
                    )
