...
class AioSandboxProvider(SandboxProvider):
    ...
    Configuration options in config.yaml under sandbox:
        use: cellflow_community.aio_sandbox:AioSandboxProvider
    ...
    def _get_extra_mounts(self, thread_id: str | None) -> list[tuple[str, str, bool]]:
        ...
        user_skills_mount = self._get_user_skills_mount()
        if user_skills_mount:
            mounts.append(user_skills_mount)
            logger.info(f"Adding user skills mount: {user_skills_mount}")
    ...
    @staticmethod
    def _get_user_skills_mount() -> tuple[str, str, bool] | None:
        """Get the per-user skills directory mount at ``/mnt/user-skills``.

        Resolves the current user via :func:`get_effective_user_id` and
        mounts the corresponding ``users/{user_id}/skills/`` directory
        ...
        try:
            ...
            if not user_skills_dir.exists():
                return None
            return (str(user_skills_dir), "/mnt/user-skills", True)  # Read-only
        except Exception as e:
            logger.warning(f"Could not setup user skills mount: {e}")
        return None
    ...
    async def _create_sandbox_async(self, thread_id: str | None, sandbox_id: str) -> str:
        ...
        # Cold start begins here — signal the user before the (possibly slow,
        # remote) container is provisioned and polled for readiness.
