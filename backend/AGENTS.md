...
### Test suite cost & day-to-day invocation

The full `make test` / `pytest tests` run takes **~60 min** (6053 tests) because it
includes real-LLM end-to-end tests. Two tests alone consume ~92% of that wall time:

| test | ~time | why |
|------|-------|-----|
| `tests/test_client_e2e.py::TestBasicChat::test_basic_chat` | 36 min | real LLM call |
| `tests/test_client_e2e.py::TestBasicChat::test_multi_turn_stateless` | 21 min | real LLM call |

Both call the real LLM via `config.yaml` credentials (marked `@requires_llm`, auto-skipped
in CI or when `OPENAI_API_KEY` is unset). Their cost is API round-trips, not test logic -
do not try to "optimize" them. `tests/test_client_live.py` is the same category (real LLM,
module-level CI skip).

**Rule: for day-to-day runs after a change, skip the LLM e2e layer** (~3 min instead of
~60). This is the default way to run tests during development:

```bash
cd backend
PYTHONPATH=. uv run pytest tests -q --ignore=tests/test_client_e2e.py --ignore=tests/test_client_live.py
```

`-m "not requires_llm"` does **not** work - `requires_llm` is a `pytest.mark.skipif`, not a
registered marker. Use `--ignore` (verified: 6053 -> 5991 tests, ~3 min).

For faster feedback, scope to what you touched:

```bash
PYTHONPATH=. uv run pytest tests/test_the_file.py -q
PYTHONPATH=. uv run pytest tests/test_the_file.py::TestClass::test_case -q
```

Run the **full** `make test` only when your change touches the LLM/e2e integration layer
(`test_client_e2e.py`, `test_client_live.py`) or just before opening a PR.

### Analysing a recorded run (post-mortem, read-only)

Two scripts under `backend/scripts/` reconstruct what a thread actually did, from persisted
data only — they never replay a run and never call a model. They answer different questions
and neither subsumes the other: a session can be **cheap and slow** (serial LLM waits) or
**fast and expensive** (parallel subagents), and those call for opposite fixes.

```bash
cd backend
# "Where did the TOKENS go" — per-subagent token/step/read accounting, gate loops, waste
PYTHONPATH=. uv run python scripts/analyze_eligibility_run.py <thread_id> \
    [--output report.json] [--baseline previous.json]

# "Where did the TIME go, and what broke" — wall clock, latency attribution, tool failures
PYTHONPATH=. uv run python scripts/analyze_run_timing.py <thread_id> \
    [--log ../logs/gateway.log] [--output timing.json] [--failures-only]
```

Start with whichever matches the complaint ("too expensive" vs "too slow"), then read the
other before concluding — the first one is routinely misleading on its own.

**Measurement traps, all of them learned the hard way.** Each cost a wrong diagnosis before
it was pinned; the scripts now encode them, so trust the scripts over a hand query:

- **A `status=running` run reports 0 tokens.** Token columns are written at finalisation, so
  a snapshot mid-run reads empty while its events already carry real usage. Session
  `2d628340` lost 17.2M that way. `analyze_eligibility_run.py` therefore prints `from_tasks`
  (summed from `subagent.end`) next to the run-row total and warns when they disagree.
- **Subagent LLM calls have no latency instrumentation.** `RunJournal.on_llm_end` records
  `latency_ms` for the lead and middleware callers only; `executor.py` attaches just a
  `SubagentTokenCollector`. So per-step subagent duration is always *inferred*, never
  measured — do not report it as measured.
- **`subagent.step.created_at` was flush time, not step time, before 2026-08-13.**
  `SubagentStepEventBuffer` batches at `FLUSH_THRESHOLD=25`, so a 127-step task collapsed
  onto 7 distinct second values. `analyze_run_timing.py` detects that shape and emits a
  caveat instead of deriving per-step numbers from it. For those older runs, pass `--log`:
  `task_tool`'s poll lines are the only wall-clock source (5s resolution).
- **Retry tasks emit no `Started background task` line.** `task_tool` re-enters its loop via
  `executor.execute_async`, so a retry's start is only in the executor's
  `starting async execution, task_id=` line. Keying on the former silently drops every retry.
- **The log is UTC+8, the database is UTC.** `logging_config.py` pins `Asia/Shanghai` for
  `%(asctime)s`; storage stays UTC (`timestamptz`). `analyze_run_timing.py` reconciles the
  two and renders UTC+8 so its output lines up with the log you are reading.
- **Tool failures are payloads, not exceptions, and not always `Error:`-prefixed.** A
  non-zero `bash` exit lands *after* the command's own stdout; a traceback and an argparse
  usage dump look different again. Matching only the prefix, on only the subagent channel,
  found 4 of 15 real failures in `a7c19ea1` — the lead channel (`llm.tool.result`) has to be
  read too.
- **Lead `task` tool arguments are not in `run_events`.** To recover the real prompt a
  subagent received, decode `checkpoint_writes` with `JsonPlusSerializer`.
- **Busy time must be a union, not a sum.** Overlapping call windows would otherwise total
  more than the wall clock. The lead/subagent *overlap* is the parallelism signal: near-zero
  means the lead is idling in its poll loop, so the session is serial and dispatch is the
  lever — not faster tools.
...
11. **ReadFileDedupMiddleware** - *(optional, if `read_file_dedup.enabled`)* Within one run, a repeat `read_file` of the same `(sandbox, thread, run, task, path, range)` whose content hash is unchanged returns a short reference instead of the payload (baseline: 147 externalised `read_file` results carried only 62 distinct hashes — 63.6% of those bytes were identical re-reads). The content hash is part of the key, so an edit is a natural miss and there is no invalidation to forget; writes additionally drop the file's entries so `read → str_replace → read` still shows the edit. `task_id` is in the key because subagent contexts are isolated — without it, task B's *first* read answers "you already read this" about a payload B cannot see or retrieve. Ordered after read-before-write so it hashes the real payload; the read mark is re-read from disk, so a dedup reference still carries a valid one (kept deliberately — dropping it would force a re-read of content the model already has). Because externalisation happens in `ToolOutputBudgetMiddleware` *outside* this one, the reference resolves the first read's `.tool-results` path from the transcript by `tool_call_id`. Its wording offers read actions only and explicitly tells the model **not** to write to the file to defeat the cache
12. **ToolErrorHandlingMiddleware** - Converts tool exceptions into error `ToolMessage`s so the run can continue instead of aborting

Two mechanical policies sit between 10 and 11 (both `wrap_tool_call`-only, both default-off, both ordered *before* dedup so a refused call never reaches the sandbox and dedup's bookkeeping only records reads that happened):

- **ReadFilePolicyMiddleware** - *(optional, if `read_file_policy.enabled`)* Within one task, a repeat **whole-file** `read_file` of a path whose first whole read was ≥ `min_lines_for_ranged` (default 1500) lines is refused (`mode: block`) with the cheaper substitute named — `grep` for line numbers, then `read_file(start_line, end_line)` — or flagged (`mode: warn`). Ranged reads are never blocked; that is the behaviour it steers toward. This is the waste `ReadFileDedupMiddleware` structurally cannot catch: dedup keys on `(path, range, hash)`, so "read whole, read whole again, then read ranges" produces a new key nearly every time (session `88df83a8`: `dedupable_read_calls=1` against `whole_file_read_calls=10`, the same 7,604-line `ocr_records.md` read whole 6 times, input tokens reaching 1.66× the compaction trigger, four compactions inside one task). Keyed per `task_id`, and an errored first read is not recorded so one transient failure cannot refuse the file for the rest of the task
- **BashWritePolicyMiddleware** - *(optional, if `bash_write_policy.enabled`)* Refuses inline-bash writes to structured artifacts (`.json` by default) so they can only be written by `write_file` / `apply_json_patches` — the tools whose payload lands in the transcript, which the read-before-write gate covers, and (for patches) which validate the document. Fires only when **all three** hold: inline authorship (`python -c`, heredoc) or a redirection/`tee`/`sed -i`; a governed artifact path (under `governed_prefix`, or relative — the sandbox cwd lives under it, so `cd /mnt/user-data/workspace && python3 -c "...open('a.json','w')..."` cannot slip through); and a real write intent (`open(...,'w')`, `json.dump`, `.write(`, ...). Running a script *file* that produces the artifact (`python3 judge_pack.py --out x.json`) is never touched — that is the main path. Deliberately not in `sandbox/tools.py`'s bash validators, which run only for the local sandbox
...
16. **SummarizationMiddleware** - *(optional, if enabled)* Context reduction when approaching token limits. `DeerFlowSummarizationMiddleware` has oscillation / low-quality guards (monitoring-optimization #2/#3): a per-(thread, run) `cooldown_calls` window, a `min_messages_to_summarize` threshold, and a `_is_low_quality_summary` gate. **These guards ship DISABLED** (`__init__` defaults and `config.yaml` are all 0) - they were reverted after monitoring session `d5fe20ec` (see `docs/eligibility-screener-monitoring-session-d5fe20ec.md`) pending independent production validation, because the #3 quality gate can cause summarization to fire repeatedly without ever compacting if the summary model persistently returns low-quality output. The code and `SummarizationConfig` fields are retained; re-enable by setting non-zero values in `config.yaml`.
17. **TodoListMiddleware** - *(optional, if `is_plan_mode`)* Task tracking with the `write_todos` tool. `TodoMiddleware` extends the base with context-loss detection (injects a reminder when `write_todos` has been truncated from the window) and premature-exit prevention. (A `write_todos` content-dedup wrapper from monitoring-optimization #10 was reverted - it broke `write_todos` in the production async path via a `ToolRuntime` injection failure.)
...
**Interface**: Abstract `Sandbox` with `execute_command(command, env=None)`, `read_file`, `write_file`, `list_dir`. The optional `env` injects per-call environment variables (request-scoped secrets — see Request-Scoped Secrets below); `LocalSandbox` merges it via `subprocess.run(env=...)` and `AioSandbox` routes env-bearing commands through the `bash.exec(env=...)` API on a fresh session. `ls_tool` post-filters `list_dir` output with `_filter_hidden_lines` (monitoring-optimization #7-A) to hide dotfile entries - in particular the `.tool-results/` directory where `ToolOutputBudgetMiddleware` externalizes large tool outputs - so the agent does not mistake externalized intermediates for deliverables; listing a dotfile directory directly (escape hatch) still shows its contents.
...
- `apply_json_patches` - Several edits to one file atomically under a single `expected_hash` version check (one lock, one read, one write). **Two patch forms, not mixable in one call**: `{old_str, new_str}` text replacement (any file; each `old_str` must match exactly once) and `{pointer, op, value}` JSON object edit (RFC 6901 pointer + `replace`/`add`/`remove`/`get`). The object form exists because a repair is a multi-field update on one entry (`conclusion` + `reason` + `evidence` + `exclusion_triggered` + `summary` counters), and locating those by string match is how "changed the reason, forgot the conclusion" happens — the eligibility skills' repair rules therefore mandate it over `str_replace` for `.json` artifacts. `get` reads one member without writing, so re-checking an entry does not need a whole-file read. Object edits preserve the file's existing indent and do not escape non-ASCII, keeping the diff limited to the changed members; `replace` never creates a missing member, `add` requires an existing parent, and a non-JSON file is rejected outright (fix broken syntax with `str_replace` first). Exempt from the read-before-write gate because `expected_hash` is the stronger check, and content-sensitive for loop detection (its args are only `path` + `patches`, so hashing salient fields alone would collapse distinct batches)
...
**Runtime guards (all OFF by default in code, `subagents.*`)**: the lead agent has had loop detection, summarization and a token budget from the start; `build_subagent_runtime_middlewares` had none of the three, which is how one judgment task reached 6.36M tokens over 83 steps (nothing compacted its context, capped its spend, or broke its 12th identical gate-script call). Opt in per deployment — **this repo's `config.yaml` arms all three**, and `tests/test_subagent_guards_are_live_in_config.py` asserts they stay armed. That test exists because a guard commented out reads exactly like a guard configured: the `subagents.token_budget` block carried a complete rationale from session `2812eaf8` onward while `#`-commented the whole time, and session `247a535f` paid for it — three judgment subagents burned 6.33M tokens with zero `write_file` calls and nothing capping them until the account hit `402 Insufficient Balance`.
- `subagents.loop_detection.enabled` attaches `LoopDetectionMiddleware` with thresholds from the **top-level** `loop_detection` section (the `verification_*` gate-script exemption was removed 2026-08-19; the mutation-aware reset below is its principled replacement). `mutation_reset` (default `true` for subagents) restarts a call set's repeat counter when a *different* call set mutated state since its last occurrence - `sha256sum -> apply_json_patches -> gate -> sha256sum` is prescribed workflow, not a loop. The reset is self-excluding (`rm -rf x` x5 still hard-stops) and budgeted per hash (`mutation_reset_budget`, default 8) so alternating writers cannot reset each other forever. `cumulative_counting` (default `true` for subagents) counts identical call sets per scope instead of only inside the sliding window — a subagent interleaves reads/greps between repeats, so the 20-call window evicts the hash before the counter reaches the limit. Counters are scoped per `(thread, run, task)`; the lead agent keeps its historical per-thread scope and window-only counting.
- `subagents.summarization` is a full `SummarizationConfig`, separate from the lead's, because a subagent is one long task rather than a conversation. Construction is shared via `summarization_middleware.build_summarization_middleware` so the two cannot drift.
- **The subagent gets its compaction summary back at model-call time.** `_maybe_summarize` parks the summary in `state["summary_text"]` instead of inserting it into `messages`, and the only renderer of that channel — `DurableContextMiddleware` — is lead-only. So subagent compaction was a *net deletion*: messages removed, replacement summary written to a channel nobody read. `DeerFlowSummarizationMiddleware.wrap_model_call` now injects it as a hidden `<task_progress_summary>` block when `context["is_subagent"]` is set (`subagents.summarization.inject_summary_message`, default **on** — unlike the other subagent guards, because switching it off restores silent context loss rather than disabling a feature). Injection happens per model call rather than as a state message so the summary cannot be re-summarized into itself, double-counted by `_messages_for_trigger_count`, or inserted between the AI/tool pairs that `_preserve_task_head` and `DanglingToolCallMiddleware` depend on — and `wrap_model_call` adds no graph node, so the `recursion_limit`-to-turn ratio is unchanged. Failure that motivated it: session `88df83a8`, where four compactions inside one EX judgment task were followed by the subagent abandoning its assignment, inventing a `current_qc_report.json` to "review", and writing `qc_review_report.json` instead of the judgment draft — then returning `completed` (see `docs/eligibility-screener-subagent-context-and-artifact-gate-changelog.md`).
- `subagents.token_budget` is scoped **per task** (`TokenBudgetMiddleware._get_run_id` prefers `context["task_id"]`): keying on `run_id` would give every task of a run one shared allowance. Hitting the limit strips `tool_calls`, forcing a bounded partial answer instead of an unbounded burn. Note the accounting: it sums raw `usage_metadata`, and `input_tokens` re-bills the whole context every step, so a 90-step task with a steady 60k context legitimately reaches ~5M — `max_tokens` bounds *total spend on this task*, not context size. Set `hard_stop_threshold` below 1.0 so the model still has budget left to write its artifact; stopping at 100% yields the same nothing as having no budget.
- **Compaction must free more context than the reading set costs.** `_determine_cutoff_index` (LangChain parent) sizes the preserved suffix against the whole `keep` budget, and `_preserve_task_head` then moves the head back into the preserved set — so the real post-compaction size was `keep + head`. `_keep_budget_after_head` charges the head to the budget (`_determine_cutoff_index_for`, floored at `_MIN_SUFFIX_TOKEN_SHARE` of `keep` so a huge head cannot collapse the working window). Without it, session `247a535f` ran `trigger` 60k / `keep` 40k with a 13–15k head: compaction landed at ~55k against a 60k trigger, the next tool result re-crossed it, and one task compacted **14 times** with the first pass freeing 4.5k (60,800 → 55,786). Worse, `read_file_dedup` then saw its first reads fall out of context and re-sent full payloads (50 pass-throughs in the judgment phase), so the saved tokens came straight back. Corollary for tuning: **`trigger` must exceed the context a task needs just to start working** — for judgment subagents the mandatory reading set alone is ~50–60k, so a 60k trigger fired at step 2 and the task never reached its first `write_file`.
- **An oversized task head degrades, it does not cancel the rescue.** `_preserve_task_head` used to bail out entirely when the head exceeded half the `keep` budget, which made a *larger* contract *more* likely to be lost. It now keeps the first `HumanMessage` (the assignment) and lets the skill `SystemMessage` block be summarized: rules can be re-read from disk by path, an assignment cannot. In `247a535f` all three judgment subagents lost their identity after the first compaction — one concluded it was "apparently the main agent", the others read files their prompt forbade — and finished 93–118 steps with zero `write_file` calls.
- **The injected summary must deny being a handover.** `_SUBAGENT_SUMMARY_NEGATIVE_CONTRACT` states outright that the block is the subagent's *own* compacted steps, that no other agent or prior attempt exists, and that the task statement wins on conflict. Session `247a535f`: the subagents read their own summary as second-hand testimony and argued with it — "according to the handover, the previous sub-agent found the OCR records were empty/incomplete" (no handover, no previous subagent, OCR complete), costing eight steps to disprove a self-invention. The framing was partly self-inflicted: the configured `summary_prompt` called its output a 交接单 ("handover note"), so prompts feeding this middleware must avoid handover/third-party phrasing.
**Per-task context key**: `_aexecute` injects `context["task_id"] = result.task_id`. All tasks of a run share thread/run/sandbox ids, so this is the only isolation boundary available to run-scoped middleware caches — without it `ReadFileDedupMiddleware` answers task B's *first* read with "you already read this" about task A's read, which B's isolated context cannot see or retrieve.
**An assignment may be passed by path**: `task(prompt_file=...)` accepts an absolute path under `/mnt/user-data/` (≤ `PROMPT_FILE_MAX_CHARS`) whose contents become the subagent's prompt; `prompt` and `prompt_file` are mutually exclusive and one is required. Both are resolved at the tool boundary (`_resolve_prompt_source`) so a bad path costs no task allowance, and the same prefix rule as `expected_outputs` applies for the same reason — a path outside user-data would turn delegation into a host-file read. Skill paths are excluded deliberately: a skill file holds the *template* with placeholders unresolved, not an assignment. This exists because two rules collided. Delegation templates must reach the subagent **verbatim** (session `9a83ccc9`: the lead paraphrased a 12.7k-char template down to 1.8k, dropped the `check_judgment_structure.py` gate command, and the subagent invented its own output schema), but making the lead retype one is billed per token — in session `247a535f` the three-way judgment dispatch was the slowest lead call of the run at **143.6s / 15,265 output tokens**, all of it fixed template text. Rendering the template mechanically (`eligibility-judgment/scripts/render_judge_prompt.py`) and passing a path is *more* faithful than hand-copying, since no model re-emits the bytes.

**Declared artifacts are verified before a task may report success**: `task(expected_outputs=[...])` takes absolute sandbox paths under `/mnt/user-data/` (≤ 10, validated at the tool boundary so a malformed declaration costs no task allowance). `SubagentExecutor._verify_expected_outputs` probes each one with `asyncio.to_thread(sandbox.download_file, path, max_bytes=4096)` — the only provider-agnostic existence check, since its contract requires both the local and remote sandbox to raise `OSError` for a missing path (`list_dir` returns host-resolved paths locally and never compares equal to the virtual path). `max_bytes` is a **size limit, not a read window** — every implementation raises `OSError(EFBIG)` rather than returning a prefix — so `EFBIG` is the one errno that *proves* the artifact exists and holds more than the probe window, and it passes; only `ENOENT` and other errnos mean missing. Reading `EFBIG` as "absent" made the gate fire on every artifact larger than 4 KB, i.e. on precisely the structured JSON it exists to guard: in session `9f069246` all three parse tasks were failed while their output sat on disk (`criteria_parsed_IN.json` 32 KB, `criteria_parsed_EX.json` 56 KB), burning two IN retries (1.55M + 0.49M tokens). The bug shipped green because the test double implemented `max_bytes` as truncation (`payload[:max_bytes]`), inverting the contract it was standing in for — test doubles for `Sandbox` must raise `OSError(EFBIG)` on oversize. A missing or empty (`{}` / `[]` / whitespace) artifact turns `COMPLETED` into `FAILED` with the paths named and `stop_reason=None`, so `task`'s existing single retry applies. No declaration, no sandbox state, or any provider error **skips** the check — a verification step that can fail a task on its own infrastructure problems is worse than none. Without it, session `88df83a8` accepted a subagent's `completed` claim for a judgment draft that was never written and the lead only noticed 8 minutes later by running the structure gate itself.

**Resource-ceiling failures are not retried**: `SubagentResult.stop_reason` (set from `subagents/stop_reasons.py::classify_stop_reason`, e.g. `recursion_limit`) is persisted to `subagent.end.metadata.stop_reason` and consulted by `task_tool._is_retryable_failure`. `SUBAGENT_MAX_RETRIES = 1` no longer applies to an exhausted allowance — re-running spends the same allowance and fails the same way, which is exactly why timeouts were never retried (session `d393714d`: a 6.36M-token failure retried for another 5.21M). Set `subagents.graceful_stop.retry_resource_ceiling_failures: true` for the old behaviour.
