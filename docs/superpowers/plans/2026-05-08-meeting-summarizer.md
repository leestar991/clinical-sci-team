# meeting-summarizer Agent 实施计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 在 `backend/.deer-flow/agents/meeting-summarizer/` 创建学术会议情报分析协调器 agent，接收会议 MD 材料和照片，通过五阶段协议输出深度研究报告和汇报 PPT，并维护跨会议时间线档案与主题知识图谱。

**Architecture:** 单层 orchestrator 模式，`meeting-summarizer` 作为主协调器，复用现有子团队（`literature-analyzer`、`data-extractor`、`report-writer`、`sci-ppt-generator`）。两阶段关键词确认协议确保检索参数经用户验证后再执行穷尽检索。知识库维护采用追加式时间线 + 按主题合并的双格式结构。

**Tech Stack:** DeerFlow agent 配置（`config.yaml` + `SOUL.md` + `memory.json`）；工具组 `web`、`file:read`、`file:write`、`bash`；子团队 `task()` 并行调用（≤3并发）；`ask_clarification` 两阶段确认；`sips` 图片压缩预处理；`write_todos` 任务追踪。
...
| 操作 | 路径 | 职责 |
|------|------|------|
| 创建 | `backend/.deer-flow/agents/meeting-summarizer/config.yaml` | Agent 模型、工具组、允许子团队声明 |
| 创建 | `backend/.deer-flow/agents/meeting-summarizer/SOUL.md` | 角色定义、五阶段协议、知识库维护规范、行为约束 |
| 创建 | `backend/.deer-flow/agents/meeting-summarizer/memory.json` | 空初始记忆结构（与现有 agent 格式一致） |
...
**Files:**
- Create: `backend/.deer-flow/agents/meeting-summarizer/config.yaml`

- [ ] **Step 1：在目标路径创建 config.yaml**

```yaml
name: meeting-summarizer
description: 学术会议情报分析协调器 — 解析会议材料与照片，穷尽检索同靶点/通道/适应症竞品与专利，输出深度研究报告和汇报PPT，并维护跨会议知识库。
...
写入路径：`backend/.deer-flow/agents/meeting-summarizer/config.yaml`

- [ ] **Step 2：验证文件可被解析**

```bash
cd /Volumes/data/github/clinical-sci-team/backend
python3 -c "import yaml; cfg=yaml.safe_load(open('.deer-flow/agents/meeting-summarizer/config.yaml')); print('name:', cfg['name']); print('subagents:', cfg['allowed_subagents'])"
```

期望输出：
```
name: meeting-summarizer
subagents: ['literature-analyzer', 'data-extractor', 'report-writer', 'sci-ppt-generator']
```

- [ ] **Step 3：提交**

```bash
git add backend/.deer-flow/agents/meeting-summarizer/config.yaml
git commit -m "feat(meeting-summarizer): add agent config"
```

---

## Task 2：创建 `memory.json`

**Files:**
- Create: `backend/.deer-flow/agents/meeting-summarizer/memory.json`

- [ ] **Step 1：创建空初始记忆文件**

写入路径：`backend/.deer-flow/agents/meeting-summarizer/memory.json`
...
```bash
python3 -c "import json; d=json.load(open('backend/.deer-flow/agents/meeting-summarizer/memory.json')); print('valid, facts count:', len(d['facts']))"
```
...
```bash
git add backend/.deer-flow/agents/meeting-summarizer/memory.json
git commit -m "feat(meeting-summarizer): add empty memory.json"
```

---

## Task 3：创建 `SOUL.md` — 身份与核心协议

**Files:**
- Create: `backend/.deer-flow/agents/meeting-summarizer/SOUL.md`

- [ ] **Step 1：写入 SOUL.md 身份与子团队能力图谱部分**

写入路径：`backend/.deer-flow/agents/meeting-summarizer/SOUL.md`
...
```bash
wc -l backend/.deer-flow/agents/meeting-summarizer/SOUL.md
```
...
## Task 4：扩展 `SOUL.md` — 五阶段执行协议

**Files:**
- Modify: `backend/.deer-flow/agents/meeting-summarizer/SOUL.md`（追加内容）

- [ ] **Step 1：追加五阶段协议到 SOUL.md**
...
批次1（并行，≤3）：
  ...
  task(description="MNC官网与投资者材料检索", prompt="...[web search]...")  ← 直接调用web
...
3. 若已存在：在文件中找到"会议记录"节，追加本次会议条目；若竞品表格中已有该药物，更新该行而非重复追加
...
- [ ] **Step 2：验证 SOUL.md 完整性**

```bash
wc -l backend/.deer-flow/agents/meeting-summarizer/SOUL.md
grep -c "阶段" backend/.deer-flow/agents/meeting-summarizer/SOUL.md
```

期望输出：行数 > 200，"阶段"出现次数 ≥ 5

- [ ] **Step 3：提交**

```bash
git add backend/.deer-flow/agents/meeting-summarizer/SOUL.md
git commit -m "feat(meeting-summarizer): add SOUL.md with 5-phase execution protocol"
```
...
**Files:**
- 不新增文件；验证 Task 1-4 产物的一致性

- [ ] **Step 1：验证 agent 目录结构完整**

```bash
find backend/.deer-flow/agents/meeting-summarizer -type f | sort
```
...
- [ ] **Step 2：验证 config.yaml 子团队与 SOUL.md 中引用的子团队完全一致**

```bash
python3 -c "
import yaml
cfg = yaml.safe_load(open('backend/.deer-flow/agents/meeting-summarizer/config.yaml'))
declared = set(cfg['allowed_subagents'])
print('config.yaml 声明的子团队:', sorted(declared))

soul = open('backend/.deer-flow/agents/meeting-summarizer/SOUL.md').read()
for sa in ['literature-analyzer', 'data-extractor', 'report-writer', 'sci-ppt-generator']:
    found = sa in soul
    print(f'  {sa} in SOUL.md:', found)
"
```

期望输出：所有4个子团队均在 SOUL.md 中出现，且与 config.yaml 声明一致。

- [ ] **Step 3：验证 memory.json 结构合法且与现有 agent 格式兼容**

```bash
python3 -c "
import json
new_mem = json.load(open('backend/.deer-flow/agents/meeting-summarizer/memory.json'))
ref_mem = json.load(open('backend/.deer-flow/agents/biostats/memory.json'))
# 检查顶层 key 一致
new_keys = set(new_mem.keys())
ref_keys = set(ref_mem.keys())
print('顶层 key 一致:', new_keys == ref_keys)
print('new keys:', sorted(new_keys))
print('ref keys:', sorted(ref_keys))
"
```

期望输出：`顶层 key 一致: True`

- [ ] **Step 4：验证 SOUL.md 包含所有必要章节**
...
missing = [r for r in required if r not in soul]
if missing:
    print('缺失章节/关键词:', missing)
else:
    print('所有必要内容均存在 ✓')
"
```
...
```bash
git add backend/.deer-flow/agents/meeting-summarizer/
git status
git commit -m "feat(meeting-summarizer): complete agent files - config, SOUL, memory"
```
...
- [x] **Spec 覆盖**：
  - 五阶段协议 ✓（Task 4）
  - 两阶段关键词确认 ✓（阶段2，ask_clarification 格式完整）
  - 七类检索站点 ✓（阶段3，逐一列出）
  - 报告八章节结构 ✓（阶段4，完整模板）
  - 双格式知识库（时间线 + 主题图谱）✓（阶段5，含初始化逻辑）
  - 图片压缩预处理 ✓（图片预处理协议，含 sips + 备用 convert）
  - 错误处理6种场景 ✓（错误处理表格）
  - 方案B/C记录 ✓（spec 文档中已记录，本 agent 为方案A实现）
- [x] **无占位符**：所有代码块、命令、期望输出均完整
- [x] **类型一致**：SOUL.md 中 subagent_type 字符串与 config.yaml `allowed_subagents` 完全对应
- [x] **并发约束**：SOUL.md 中明确 ≤3 并发，与系统 `MAX_CONCURRENT_SUBAGENTS=3` 一致
